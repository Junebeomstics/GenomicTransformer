...
class FactorizedLoss(BaseLoss):
    ...
    def forward(self, y_hat, y):
        ...
        l = torch.mean(nll) * (ny / (ny-padding_size))
        self.cum_loss += l
        return l
...
class PlainLoss(BaseLoss):
    ...
    def get_description(self, step):
        ...
        desc = " token loss : %f, token ppl : %f," % (
            tok_loss / step, math.exp(tok_loss / step))
        return desc


class StructuredLoss(BaseLoss):
    def __init__(self, token_padding_index:int, structure_padding_index:int, token_loss='experimental'):
        super(StructuredLoss, self).__init__()
        if token_loss =='experimental':
            self.tl = FactorizedLoss(token_padding_index)
        elif token_loss =='plain':
            self.tl = PlainLoss(token_padding_index)
        self.sl = PlainLoss(padding_idx=structure_padding_index)

    def forward(self, net_outs, Ys):
        y_hat, s_hat = net_outs
        y, s = Ys

        bs, l = s_hat.size()[:2]
        # print(y_hat.size(), s_hat.size(), y.size(), s.size())
        y_loss = self.tl(y_hat, y.contiguous().view(-1))
        s_loss = self.sl(s_hat.contiguous().view(bs*l,-1), s.contiguous().view(-1))
        return y_loss + s_loss

    def get_description(self, step):
        tok_loss = self.tl.cum_loss
        st_loss = self.sl.cum_loss
        desc = "tot loss : %f, token loss : %f, token ppl : %f, structure loss : %f," % (
            (tok_loss + st_loss) / step, tok_loss / step, math.exp(tok_loss / step), st_loss / step)
        return desc
